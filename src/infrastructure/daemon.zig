...
const TickFn = *const fn (*anyopaque) anyerror!void;
const StartFn = *const fn (*anyopaque) anyerror!void;
...
pub const Daemon = struct {
    ...
    pub fn run(self: *Daemon) !void {
        ...
        if (self.opts.on_start) |f| {
            if (self.opts.ctx) |ctx| try f(ctx);
        }

        while (!self.shouldStop()) {
            if (self.opts.tick) |tick_fn| {
                if (self.opts.ctx) |ctx| try tick_fn(ctx);
            } else {
                std.Thread.sleep(self.opts.idle_ns);
            }
        }

        if (self.opts.on_stop) |f| {
            if (self.opts.ctx) |ctx| f(ctx);
        }
        self.running = false;
    }
    ...
    fn shouldStop(self: *Daemon) bool {
        return self.stop_requested or stop_flag.load(.seq_cst);
    }
    ...
    fn installHandlers(self: *Daemon) !void {
        ...
        posix.sigaction(posix.SIG.INT, &action, &self.prev_int);
        posix.sigaction(posix.SIG.TERM, &action, &self.prev_term);
        self.installed_handlers = true;
    }

    fn restoreHandlers(self: *Daemon) void {
        if (!self.installed_handlers) return;
        posix.sigaction(posix.SIG.INT, &self.prev_int, null);
        posix.sigaction(posix.SIG.TERM, &self.prev_term, null);
